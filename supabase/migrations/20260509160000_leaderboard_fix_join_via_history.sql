-- Fix leaderboard queries to resolve user_id via workout_history rather than
-- workout_sets.user_id, which may be NULL on older rows that pre-date the
-- column being added. workout_history.user_id is always populated.
...
LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  WITH user_heaviest AS (
    SELECT wh.user_id, MAX(ws.weight) AS heaviest_weight
    FROM workout_sets ws
    JOIN workout_history wh ON wh.id = ws.workout_history_id
    LEFT JOIN profiles p ON p.user_id = wh.user_id
    WHERE ws.exercise_id = p_exercise_id
  ...
  max_reps_at_heaviest AS (
    ...
    FROM workout_sets ws
    JOIN workout_history wh ON wh.id = ws.workout_history_id
    JOIN user_heaviest uh ON uh.user_id = wh.user_id AND uh.heaviest_weight = ws.weight
    WHERE ws.exercise_id = p_exercise_id
    ORDER BY wh.user_id, ws.reps DESC, ws.created_at DESC
  )
  SELECT
