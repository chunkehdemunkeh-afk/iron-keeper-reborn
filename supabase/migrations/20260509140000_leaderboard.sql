-- Leaderboard feature
-- Adds opt-out column to profiles and cross-user RPC functions for rankings
...
-- ── 2. RLS: allow cross-user profile reads for leaderboard display ────────────
-- Authenticated users can see any profile where leaderboard_visible = true,
-- plus their own profile regardless.

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'profiles' AND policyname = 'leaderboard_profiles_select'
  ) THEN
    CREATE POLICY "leaderboard_profiles_select" ON profiles
      FOR SELECT TO authenticated
      USING (leaderboard_visible = true OR auth.uid() = user_id);
  END IF;
END $$;

-- ── 3. RPC: top 20 most-logged exercises across all visible users ─────────────
...
LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  ...
  JOIN profiles p ON p.user_id = ws.user_id AND p.leaderboard_visible = true
...
-- ── 4. RPC: 1RM leaderboard (Epley estimate, prefers actual 1rm_test singles) ─
...
LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  ...
  JOIN profiles p ON p.user_id = bpu.user_id AND p.leaderboard_visible = true
  ORDER BY best_1rm DESC;
$$;

-- ── 5. RPC: max weight leaderboard ───────────────────────────────────────────
...
LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  ...
  FROM best_set bs
  JOIN profiles p ON p.user_id = bs.user_id AND p.leaderboard_visible = true
  ORDER BY max_weight DESC;
$$;

-- ── 6. RPC: max reps at heaviest weight leaderboard ──────────────────────────
...
LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  ...
  JOIN profiles p ON p.user_id = mr.user_id AND p.leaderboard_visible = true
  ORDER BY max_reps DESC, heaviest_weight DESC;
$$;

-- ── 7. RPC: session volume leaderboard ───────────────────────────────────────
...
LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  WITH session_vols AS (
    ...
    JOIN profiles p ON p.user_id = wh.user_id AND p.leaderboard_visible = true
    WHERE ws.set_type IN ('working', '1rm_test')
      AND ws.weight > 0
      AND (
        p_session_type = 'All'
  ...
  FROM best_per_user bpu
  JOIN profiles p ON p.user_id = bpu.user_id
